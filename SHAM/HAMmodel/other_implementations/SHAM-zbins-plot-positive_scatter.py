...
fileroot = '{}MCMCout/discarded_zbins_{}/{}_{}_{}_{}_z{}z{}/multinest_'.format(home,date,function,rscale,gal,GC,zmin,zmax)
...
if os.path.exists('{}best-fit_{}_{}-python.dat'.format(fileroot[:-10],gal,GC)):
    ...
    def sham_tpcf(uni,uni1,sigM,sigV,Mtrun):
        if function == 'mps':
            ...
            tpcf   = [(x00+x01)/2,(x20+x21)/2,(v0+v1)/2, (n0+n1)/2]
        else:        
    ...
    def sham_cal(uniform,sigma_high,sigma,v_high):    
        ...
        datav = datac[:,1]*scatter   # modified Vpeak_scat

        org3  = datac[(datav<v_high)]  # 4.89s
        LRGscat = org3[np.argpartition(-datav[(datav<v_high)],SHAMnum)[:(SHAMnum)]]
        datav   = datav[np.argpartition(-datav[(datav<v_high)],SHAMnum)[:(SHAMnum)]]
        ...
        return [np.sum(mono,axis=-1)/nmu,np.sum(quad,axis=-1)/nmu,min(datav),n]
    ...
    if finish: 
        ...
        xi = np.array([np.mean(xi1_ELG,axis=0)[0],np.mean(xi1_ELG,axis=0)[1]]).T
        model = xi.T.flatten()
        res = OBS-model
        ...
        f.close()
        # save python 2pcf
        np.savetxt('{}best-fit_{}_{}-python.dat'.format(fileroot[:-10],gal,GC),model.reshape(2,nbins).T,header='python chi2 = {:.3f}\n xi0 x12'.format(res.dot(covR.dot(res))))
