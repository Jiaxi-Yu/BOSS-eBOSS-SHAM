...
def prior(cube, ndim, nparams):
    if (gal=='LRG'):
        ...
        cube[2] = 1000.*cube[2]+1000.
    elif gal=='ELG':
        cube[0] = 5.*cube[0]
        cube[1] = 60.*cube[1]      
        cube[2] = 300.*cube[2]+200.
...
if mode == 'debug':
    ...
    #    return z*(x[0]+y[0])
    #with Pool(processes = nseed) as p:
    #    xi0_tmp = p.starmap(test,zip(uniform_randoms,uniform_randoms1,repeat(1)))
    #print(xi0_tmp)
    #print(sham_tpcf(uniform_randoms[0],uniform_randoms[1],0.5,100.,5.));print('a single 2pcf calculation no problem')
    print(chi2(0.5,100.,1e7))
else:
