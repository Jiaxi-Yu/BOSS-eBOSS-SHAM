...
if os.path.exists('{}best-fit_{}_{}-wp_test2.dat'.format(fileroot[:-10],gal,GC)):
    errbar = np.std(mocks,axis=1)
    if func =='mps':
        ...
        for col,covbin,name,k in zip(cols,[int(0),int(200)],['monopole','quadrupole'],range(2)):
            ...
            for j in range(2):
                ...
                ax[j,k].plot(s,s**2*(xi[1][:,k]-values[j]),alpha=0.6,label = '\$\sigma$ cut')
                ax[j,k].plot(s,s**2*(xi[2][:,k]-values[j]),alpha=0.6,label = 'pos_scatter/$\sigma$ cut')
        ...
        wp = [np.loadtxt('{}best-fit_{}_{}-wp_test{}.dat'.format(fileroot[:-10],gal,GC,x))[binmin:binmax] for x in range(3)]
    ...
    def sham_cal(uniform):
        ...
        elif cut == 'pos_dsigma': # pos_scatter-dsigma-cut
            if gal =='LRG':
    ...
    # save python 2pcf
    np.savetxt('{}best-fit_{}_{}-wp_test{}.dat'.format(fileroot[:-10],gal,GC,cut),np.array([s,model0]).T,header='python chi2 = {:.3f}/{}\n rp wp'.format(res.dot(covR.dot(res)),nbins))
