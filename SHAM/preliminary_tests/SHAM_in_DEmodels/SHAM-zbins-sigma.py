...
# calculate mean monopole in parallel
    with Pool(processes = nseed) as p:
        xi0_tmp = p.starmap(sham_tpcf,zip(uniform_randoms,uniform_randoms1,repeat(float32(sigma_M))))
...
parameters = ["sigma"]
npar = len(parameters)
fileroot = '{}MCMCout/zbins_{}/{}/multinest_'.format(home,'python',cataname)
...
if mode == 'debug':
    ...
    pymultinest.run(loglike, prior, npar,n_live_points= npoints, outputfiles_basename=fileroot, \
                    resume =True, verbose = True,n_iter_before_update=5,write_output=True)
    ...
    f.write('another way around: sigma {:.6}+{:.6}{:.6}, sigma_smear {:.6}+{:.6}{:.6}km/s \n'.format(a.get_best_fit()['parameters'][0],upper[0]-a.get_best_fit()['parameters'][0],lower[0]-a.get_best_fit()['parameters'][0]))
    ...
    def sham_tpcf1(uni,uni1,sigM):
        x00,x20= sham_cal(uni,sigM)
        x01,x21= sham_cal(uni1,sigM)
        return [append(x00,x01),append(x20,x21)]

    with Pool(processes = nseed) as p:
        xi1_ELG = p.starmap(sham_tpcf1,list(zip(uniform_randoms,uniform_randoms1,repeat(np.float32(a.get_best_fit()['parameters'][0])))))
    # xi0
