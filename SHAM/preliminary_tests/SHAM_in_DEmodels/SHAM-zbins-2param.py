        ...
        for j in range(2):
            ...
            if (j==0):
                ...
                if k==0:
                    ...
                    ax[j,k].errorbar(s+0.2,s**2*(xi[:,k]-values[j]),s**2*stdSHAM[k],c='c', marker='^',ecolor='c',ls="none",alpha=0.8,label='SHAM, $\chi^2$/dof={:.4}/{}'.format(-2*a.get_best_fit()['log_likelihood'],int(2*nbins-3)))
                    plt.legend(loc=1)
