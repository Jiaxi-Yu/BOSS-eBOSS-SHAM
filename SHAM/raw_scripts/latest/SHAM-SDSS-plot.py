...
home     = os.environ['HOME']+'/SHAM/'
scratch  = os.environ['SCRATCH']
fileroot = '{}MCMCout/zbins_{}/SDSS/{}_{}_{}_{}_z{}z{}/multinest_'.format(home,date,function,rscale,gal,GC,zmin,zmax)
...
if (rscale=='linear')&(function=='mps'):
    if gal == 'LRG':
        ...
        if (zmin == '0.2')&(zmax == '0.75'):
            SHAMnum = 208000
            z = 0.5609
            a_t = '0.64210'
        elif (zmin == '0.4')&(zmax=='0.6'):
            SHAMnum = 336564
            z = 0.51087502
            a_t = '0.65650'                
    elif gal=='CMASS':
...
# wp: log binning
binfilewp = Table.read(home+'binning_file/binfile_CUTE.dat',format='ascii.no_header')
...
# SHAM halo catalogue
if (os.path.exists('{}best-fit_{}_{}-python.dat'.format(fileroot[:-10],gal,GC)))&(not genmocks):
    ...
    halofile = scratch+'/SHAM/catalog/UNIT_hdf5/UNIT_hlist_'+a_t+'_PID.hdf5'         
    read = time.time()
    ...
    seeds1 = [1000*x   for x in range(nseed)]
    seeds2 = [1050*x+1 for x in range(nseed)]
    uniform_randoms  = [np.random.RandomState(seed=seed1).rand(len(datac)).astype('float32') for seed1 in seeds1] 
    uniform_randoms1 = [np.random.RandomState(seed=seed2).rand(len(datac)).astype('float32') for seed2 in seeds2] 

    # SHAM application
    def sham_tpcf(*par):
        if not genmocks:
            if date == '0729':
                x00,x20,v0,n0,wp0= sham_cal(par[0],par[2],par[4],par[5])
                x01,x21,v1,n1,wp1= sham_cal(par[1],par[3],par[4],par[5])
            else:
                x00,x20,v0,n0,wp0= sham_cal(par[0],par[2],par[4],par[5],par[6])
                x01,x21,v1,n1,wp1= sham_cal(par[1],par[3],par[4],par[5],par[6])
            return [append(x00,x01),append(x20,x21),(v0+v1)/2, (n0+n1)/2, append(wp0,wp1)]
        else:
            if date == '0729':
                sham_cal(par[0],par[2],par[4],par[5])
                sham_cal(par[1],par[3],par[4],par[5])
            else:
                sham_cal(par[0],par[2],par[4],par[5],par[6])
                sham_cal(par[1],par[3],par[4],par[5],par[6])
            print('SHAM mocks generated')
            return None

    def sham_cal(*PAR):
        # scatter Vpeak
        if date == '0729':
            uniform,seed,sigma_high,sigma = PAR
            v_high = 0
        else:
            uniform,seed,sigma_high,sigma,v_high = PAR
        ...
        z_redshift  = (LRGscat[:,-1]+(LRGscat[:,-5]+append(sigma*sqrt(-2*log(uniform[:scathalf]))*cos(2*pi*uniform[-scathalf:]),sigma*sqrt(-2*log(uniform[:scathalf]))*sin(2*pi*uniform[-scathalf:])))*(1+z)/H)
        z_redshift %=boxsize

        if genmocks:
            # save the best-fitted catalogue
            if not os.path.exists(f'{scratch}/SHAM/catalog/UNIT_SHAM/SHAM_{gal}_z{zmin}z{zmax}_at{a_t}_seed{seed}.dat'):
                np.savetxt(f'{scratch}/SHAM/catalog/UNIT_SHAM/SHAM_{gal}_z{zmin}z{zmax}_at{a_t}_seed{seed}.dat',np.array([LRGscat[:,-3],LRGscat[:,-2],LRGscat[:,-1],(LRGscat[:,-1]+LRGscat[:,-5]*(1+z)/H)%boxsize,z_redshift]).T,header='X,Y,Z,Z_RSD_true,Z_RSD_obs')
            return None
        else:
            # Corrfunc 2pcf and wp
            DD_counts= DDsmu(autocorr, nthread,bins,mu_max, nmu,LRGscat[:,-3],LRGscat[:,-2],z_redshift,periodic=True, verbose=False,boxsize=boxsize)
            wp_dat   = wp(boxsize,80,nthread,binswp,LRGscat[:,-3],LRGscat[:,-2],z_redshift)
    ...
    if date == '0729':
        with Pool(processes = nseed) as p:
            xi1_ELG = p.starmap(sham_tpcf,list(zip(uniform_randoms,uniform_randoms1,seeds1,seeds2,repeat(np.float32(a.get_best_fit()['parameters'][0])),repeat(np.float32(a.get_best_fit()['parameters'][1])))))
    else:
        with Pool(processes = nseed) as p:
            xi1_ELG = p.starmap(sham_tpcf,list(zip(uniform_randoms,uniform_randoms1,seeds1,seeds2,repeat(np.float32(a.get_best_fit()['parameters'][0])),repeat(np.float32(a.get_best_fit()['parameters'][1])),repeat(np.float32(a.get_best_fit()['parameters'][2]))))) 
...
if rscale == 'linear':
    ...
    for col,name,k in zip(cols,['monopole','quadrupole'],range(2)):
        ...
        for j in range(2):
            ax[j,k] = fig.add_subplot(spec[j,k])
            #ax[j,k].plot(s,s**2*(xi[:,k]-values[j]),c='c',alpha=0.6,label='SHAM-python')
            ax[j,k].plot(s,s**2*(Ccode[:,k+2]-values[j])/err[j],c='m',alpha=0.6,label='SHAM, $\chi^2$/dof={:.4}/{}'.format(-2*a.get_best_fit()['log_likelihood'],int(2*(len(s)-3)-npar)))
            ax[j,k].errorbar(s,s**2*(obscf[col]-values[j])/err[j],s**2*errbar[k*nbins:(k+1)*nbins]/err[j],color='k', marker='o',ecolor='k',ls="none",label='{} obs 1$\sigma$'.format(obstool))
            plt.xlabel('s (Mpc $h^{-1}$)')
            if rscale=='log':
                plt.xscale('log')
    ...
    plt.savefig('{}cf_{}_bestfit_{}_{}_{}-{}Mpch-1.png'.format(fileroot[:-10],multipole,gal,GC,2,rmax))
    plt.close()
