...
def plot_deltav_hist(INFO,target,zrange,max_dv=500., min_deltachi2=9, nsubvolume = 1000,title=None, save=0,coloursel=False):
    ...
    for info,GC,colour in zip(INFO,['NGC','SGC'],['r','b']):
        dc = info['delta_chi2']
        dv = info['delta_v']
        z = info['z']
        zerr = info['zerr']
        ...
        plt.plot(BIN,dens,color=colour,label = GC)
        plt.legend(loc=1)
    plt.savefig(home+'LOWZ_{}_NGC-vs-SGC.png'.format(zrange))
    plt.close()
...
def plot_all_deltav_histograms(spall,proj,zmin,zmax,target='LRG',dchi2=9,maxdv=500,spec1d=0, redrock=0,GC='NGC+SGC',coloursel=False):
    ...
    print('SDSS III comprises {:.3f}% of the repetitive data for LOWZ at {}<z<{}'.format(len(sp['MJD'][sp['MJD']>=55176])/len(sp['MJD'])*100,zmin,zmax))
    
    infoN = get_delta_velocities_from_repeats(sp,proj,target,zmin,zmax,spec1d=1,GC='NGC')
    infoS = get_delta_velocities_from_repeats(sp,proj,target,zmin,zmax,spec1d=1,GC='SGC')

    plot_deltav_hist([infoN,infoS],target,zrange='z{}z{}'.format(zmin,zmax),min_deltachi2=dchi2,  max_dv=maxdv,title='{} {} {}<z<{}'.format(proj,target,zmin,zmax), save='{}/{}-{}-repeats-{}-dchi2_{}-z{}z{}-{}.png'.format(home,proj,target,'spec1d',dchi2,zmin,zmax,GC),coloursel=coloursel)
...
for zmin,zmax,maxdv in zip(zmins,zmaxs,maxdvs):
    plot_all_deltav_histograms(repeatname,'BOSS',zmin,zmax,target='LOWZ',dchi2=9,spec1d=1,maxdv=maxdv,GC=GC,coloursel=False)
