...
for mocktype in ['sys']:
    for mockweight in ['_FKP','_FKPCP','_FKPSYSTOT','_FKPNOZ']:#['_FKP','_FKPCP','_FKPNOZ','_FKPSYSTOT']:
        ...
        flagwp = mockFITS[-1]+'wp_pi80.fits.gz'
        for k,mockdir,mockfits in zip(range(znum),mockDIR,mockFITS): 
            ...
            elif (function == 'wp')&(not os.path.exists(flagwp)):
                if mocktype == 'sys':
...
if os.path.exists(flagwp):
    for M,mockweight in enumerate(['_FKP','_FKPCP','_FKPNOZ','_FKPSYSTOT']):
        ...
        for k,pimax in enumerate(pimaxs):
            hdu = fits.open('{}nosys_FKP/EZmocks_{}_{}_{}_z{}z{}_wp_pi{}.fits.gz'.format(datapath,'nosys',function,rscale,Zrange[0],Zrange[1],pimax)) #
            mocks = hdu[1].data['NGC+SGCmocks']
            Nmock = mocks.shape[1] 
            hdu.close()
            errbarnosys = np.std(mocks,axis=1)
            ...
            for j in range(2):
                ...
                hdu = fits.open('{}sys{}/EZmocks_{}_{}_{}_z{}z{}_wp_pi{}.fits.gz'.format(datapath,mockweight,'sys',function,rscale,Zrange[0],Zrange[1],pimax)) #
                mocks = hdu[1].data['NGC+SGCmocks']
                Nmock = mocks.shape[1] 
                hdu.close()
                errbarsys = np.std(mocks,axis=1)
        ...
        plt.savefig('wp_sys-vs-nosys{}.png'.format(mockweight))
        plt.close()