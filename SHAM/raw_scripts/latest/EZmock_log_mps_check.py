...
# plot the mocks vs observations
mean = np.nanmean(mocks,axis=-1)
disp = np.nanstd(mocks,axis=-1)    
fig = plt.figure(figsize=(14,8))
spec = gridspec.GridSpec(nrows=2,ncols=2, height_ratios=[4, 1], hspace=0.2,wspace=0.1)
ax = np.empty((2,2), dtype=type(plt.axes))
for covbin,name,k in zip([int(0),int(Nstot)],['monopole','quadrupole'],range(2)):
    values=[np.zeros(nbins),OBS[k*len(s):(k+1)*len(s)]]        
    err   = [np.ones(nbins),s**2*disp[k*nbins:(k+1)*nbins]]
    for j in range(2):
        ax[j,k] = fig.add_subplot(spec[j,k])
        # mocks mean and std
        ax[j,k].fill_between(s,s**2*((mean-disp)[nbins*k:nbins*(k+1)]-values[j])/err[j],s**2*((mean+disp)[nbins*k:nbins*(k+1)]-values[j])/err[j],label='mocks',color='c',alpha=0.4)
        ax[j,k].plot(s,s**2*(mean[nbins*k:nbins*(k+1)]-values[j])/err[j],c='c')
        ax[j,k].errorbar(s,s**2*(OBS[k*len(s):(k+1)*len(s)]-values[j])/err[j],s**2*disp[k*nbins:(k+1)*nbins]/err[j],color='k', marker='o',ecolor='k',ls="none",label='obs')
        ...
        if (j==0):
            ...
            if k==0:
                ...
                plt.legend(loc=1)
            plt.title('correlation function {} at {}<z<{}'.format(name,zmin,zmax))
        if (j==1):
            ax[j,k].set_ylabel('$\Delta\\xi_{}$/err'.format(k*2))
            plt.ylim(-3,3)

plt.savefig('mocks_vs_obs_{}_{}_z{}z{}.png'.format(gal,GC,zmin,zmax),bbox_tight=True)
plt.close()
