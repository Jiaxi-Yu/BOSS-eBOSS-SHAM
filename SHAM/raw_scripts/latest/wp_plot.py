...
for GC in ['NGC','SGC']:
    for gal,ver in zip(['LRG','ELG'],['v7_2','v7']):
        A = Table.read('{}codes/FCFC/wp_test/cut/wp_log_{}_{}_rp80.dat'.format(home,gal,GC),format='ascii.no_header')
...
for a in ['0.50320','0.52600','0.53780','0.54980']:
    ...
    binswp = np.unique(np.loadtxt('/global/homes/j/jiaxi/codes/FCFC/wp_test/binfile_test.dat').flatten())
    ...
    np.savetxt('{}codes/FCFC/wp_test/halo/wp_corrfunc_UNIT_hlist_{}.dat'.format(home,a),corrfunc)
    
# plot
dire = '/global/cscratch1/sd/jiaxi/SHAM/catalog/UNIT_hlist_{}.hdf5'
for a in ['0.50320','0.52600','0.53780','0.54980']:    
    smin,smax,smid,corrfunc = np.loadtxt('{}codes/FCFC/wp_test/halo/wp_corrfunc_UNIT_hlist_{}.dat'.format(home,a),unpack=True)
    A = Table.read('{}codes/FCFC/wp_test/halo/wp_UNIT_hlist_{}.dat'.format(home,a),format='ascii.no_header')
    ...
    plt.savefig('wp-UNIT_{}.png'.format(a),bbox_tight=True)
    plt.close()