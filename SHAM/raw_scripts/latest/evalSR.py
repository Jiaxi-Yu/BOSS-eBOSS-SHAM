...
zr = {'LRG':(zmin,zmax),'ELG':(0.6,1.6),'QSOlz':(0.6,2.1),'QSOly':(2.1,4.5),'BGS':(0.01,0.5),'ELGhiz':(1.2,1.6)}
...
def repeatvsdchi2(tf,type,nbin=1000,rng=(9,2000),mind=500,maxd=1500,chi2x=50):    
    ...
    plt.show()
    """
    # save the catalogue that has excluded the redshift failures
    datadir = '/global/cscratch1/sd/jiaxi/SHAM/catalog/'
    tc = Table(tcomp[abs(dz) < catthresh*(1+tcomp['Z_TRUTH'])])
    tc.write(datadir+'DESI_{}_redshift_uncertainty.fits.gz'.format(type), format='fits', overwrite=True)
    
    return [dz*299792./(1+tcomp['Z_TRUTH']),tcomp['Z_TRUTH'],ggzsel] #zrsel,
