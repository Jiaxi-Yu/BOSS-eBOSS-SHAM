...
with Pool(processes = nseed) as p:
    if GC=='NGC':
        ...
        xi1_ELG = p.starmap(sham_tpcf1,zip(uniform_randoms,repeat(np.float32(sig)),repeat(np.float32(91.8)),repeat(np.float32(Vceil)))) 
        a=chi21(sig,91.8,Vceil)
        print('Multinest +Vz smearing chi2: {:.7}'.format(a))
        print('Multinest chi2:',chi2(0.80037976508,1167.7109330976073))
    if GC=='SGC':
        xi_ELG = p.starmap(sham_tpcf,zip(uniform_randoms,repeat(np.float32(0.7104378528883745)),repeat(np.float32(993.8109564008059)))) 
        xi1_ELG = p.starmap(sham_tpcf1,zip(uniform_randoms,repeat(np.float32(sig)),repeat(np.float(91.8)),repeat(np.float32(Vceil))))
        a =chi21(sig,91.8,Vceil) 
        print('Multinest +Vz smearing chi2: {:.7}',a)
        print('Multinest chi2:',chi2(0.7104378528883745,993.8109564008059))
