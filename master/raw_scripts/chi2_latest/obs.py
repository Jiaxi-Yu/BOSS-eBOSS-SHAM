...
def obs(home,gal,GC,obsname,randname,obsout,rmin,rmax,nbins,zmin,zmax,Om,exist):
	...
	if (gal=='LRG')&(exist==False):
		...
		## use trapz to integrate over mu
                ...
                Table([np.linspace(0,250,251)[:-1],np.linspace(0,250,251)[1:],obs0,obs1,obs2],names=['smin','smax','mono','quad','hexa']).write(obsout,format = 'ascii',delimiter='\t',overwrite=True)
	else:
