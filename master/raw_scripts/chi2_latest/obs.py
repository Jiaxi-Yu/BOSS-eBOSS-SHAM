...
def obs(home,gal,GC,obsname,randname,obsout,rmin,rmax,nbins,zmin,zmax,Om,exist):
	...
	if (gal==LRG)&(exist==False):
		...
		print('./FCFC/2pcf -c ./FCFC/fcfc.conf -j 1 -a '+str(rmin)+' -b '+str(rmax)+' -n '+str(nbins)+' --data-z-min '+str(zmin)+' --data-z-max '+str(zmax)+' --rand-z-min '+str(zmin)+' --rand-z-max '+str(zmax)+' -m '+str(Om)+' --data '+obsname[:-5]+' --rand '+randname[:-5]+' --dd '+home+'2PCF/obs/LRG_'+GC+'.dd --dr '+home+'2PCF/obs/LRG_'+GC+'.dr --rr '+home+'2PCF/obs/LRG_'+GC+'.rr --output '+home+'2PCF/obs/LRG_'+G)
		os.system('./FCFC/2pcf -c ./FCFC/fcfc.conf -j 1 -a '+str(rmin)+' -b '+str(rmax)+' -n '+str(nbins)+' --data-z-min '+str(zmin)+' --data-z-max '+str(zmax)+' --rand-z-min '+str(zmin)+' --rand-z-max '+str(zmax)+' -m '+str(Om)+' --data '+obsname[:-5]+' --rand '+randname[:-5]+' --dd '+home+'2PCF/obs/LRG_'+GC+'.dd --dr '+home+'2PCF/obs/LRG_'+GC+'.dr --rr '+home+'2PCF/obs/LRG_'+GC+'.rr --output '+home+'2PCF/obs/LRG_'+GC+'.dat')
		...
		Table([obs0[rmin:rmax],obs1[rmin:rmax],obs2[rmin:rmax]],names=['mono','quad','hexa']).write(obsout,format = 'ascii',delimiter='\t',overwrite=True)
