...
nmu      = 120
autocorr = 1
home      = '/home/epfl/jiayu/Desktop/HAM-MCMC/'
...
# HAM application
def sham_tpcf1(uniform,N1,v_high):
    if GC == 'NGC':
    	sigma_high = 0.8003797650857304
    if GC == 'SGC':
        sigma_high = 0.7104378528883745
    datav = np.copy(V)
...
# calculate mean monopole in parallel
    with Pool(processes = nseed) as p:
        xi0_tmp = p.starmap(sham_tpcf,zip(uniform_randoms,repeat(int(N1)),repeat(np.float(v_high))))
    print('a second calculation')
    with Pool(processes = nseed) as p:
        xi1_tmp = p.starmap(sham_tpcf,zip(uniform_randoms1,repeat(int(N1)),repeat(np.float(v_high))))
...
def prior(cube, ndim, nparams):
    if gal=='LRG':
        cube[0] = 500*100**cube[0] # log between [500,50000]
        cube[1] = 900*cube[1]+600  # uniform between [600,1500]

# loglikelihood = -0.5*chi2    
def loglike(cube, ndim, nparams):
    N1,vcut = cube[0],cube[1]
    return -0.5*chi2(N1,vcut)   
...
g.triangle_plot(sample,['N1', 'Vceil'], filled=True)#,title_limit=1)
