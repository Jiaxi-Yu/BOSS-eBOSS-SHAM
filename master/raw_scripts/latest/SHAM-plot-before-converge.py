...
# HAM application
def sham_tpcf(uni,sigM,sigV,Mtrun):
...
with Pool(processes = nseed) as p:
    xi1_ELG = p.starmap(sham_tpcf,zip(uniform_randoms,repeat(np.float32(a.get_best_fit()['parameters'][0])),repeat(np.float32(a.get_best_fit()['parameters'][1])),repeat(np.float32(a.get_best_fit()['parameters'][2]))))    

np.savetxt(bestfit,np.hstack((np.mean(xi1_ELG,axis=0)[0],np.mean(xi1_ELG,axis=0)[1])))
